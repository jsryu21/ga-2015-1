...
void SteadyStateGA::LK(Solution& offspring, Solution& optOffSpring) {
    C2EdgeTour* tour = new C2EdgeTour(solutionLen);
    tour->convertFromOrder(&offspring.Chromosome[0], solutionLen);
    lk->run(tour);
    tour->convertToOrder(&offspring.Chromosome[0], solutionLen);
    delete tour;
    Normalize(offspring);
    CALL_MEMBER_FN(*this, Evaluate)(offspring);
    std::copy(offspring.Chromosome.begin(), offspring.Chromosome.end(), optOffSpring.Chromosome.begin());
...
void SteadyStateGA::GA() {
    for (int i = 0; i < PSIZE; ++i) {
        ...
        //CALL_MEMBER_FN(*this, LocalOpt)(population[i], population[i]);
    }
    InitRecords();
    if (Preprocess) {
...
void SteadyStateGA::LK_1() {
    // asume PSIZE is 100
    for (int i = 0; i < PSIZE; ++i) {
        CALL_MEMBER_FN(*this, GenerateRandomSolution)(population[i]);
    }
    for (int i = 0; i < PSIZE; ++i) {
        LK(population[i], population[i]);
    }
    for (int i = 0; i < PSIZE; ++i) {
        std::cout << population[i].Fitness << std::endl;
    }
...
void SteadyStateGA::MultiStartLK() {
    int cnt = 15000 / PSIZE;
    for (int j = 0; j < cnt; ++j) {
        double minFitness = std::numeric_limits< double >::max();
        for (int i = 0; i < PSIZE; ++i) {
...
void SteadyStateGA::PrintStatistics() {
