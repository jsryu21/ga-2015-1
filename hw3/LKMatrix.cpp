...
LKMatrix::LKMatrix(const std::vector< std::vector< double > >& edgeDistances) {
    this->edgeDistances = edgeDistances;
    size = edgeDistances.size();
}
...
void LKMatrix::OptimizeTour(const std::vector< int >& tour, std::vector< int >& tour_opt) {
    this->tour = std::vector< int >(size);
    std::copy(tour.begin(), tour.end(), this->tour.begin());
    optimizeTour();
    int current = 0;
    int index = 0;
    do {
        tour_opt[index] = this->tour[current];
        current = this->tour[current];
        ++index;
    } while (current != 0);
}
